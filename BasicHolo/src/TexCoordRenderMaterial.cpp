...
TexCoordRenderMaterial::TexCoordRenderMaterial(GLuint unit, int width, int height) : GLEngine::Material(), m_texUnit(unit) {
...
TexCoordRenderMaterial::~TexCoordRenderMaterial() {
    glDeleteTextures(1, &m_texID);
    glDeleteRenderbuffers(1, &m_buf);
    glDeleteFramebuffers(1, &m_FBO);
}

GLuint TexCoordRenderMaterial::getFBO() const noexcept {
    return m_FBO;
}